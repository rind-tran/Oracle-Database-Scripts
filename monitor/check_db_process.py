...
# check db session
def db_processes(warn, crit):
    con = oracledb.connect(user=db_config_sys.sysuser, password=db_config_sys.syspw, dsn=db_config_sys.dsn)
    cur = con.cursor()
    sql = '''
    select round(CURRENT_UTILIZATION/LIMIT_VALUE*100,2) sessions_usage from v$resource_limit where resource_name = 'processes'
    '''
    cur.execute(sql)
    res = cur.fetchall()
    res, = res[0]
